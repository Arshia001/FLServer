...
let runUpdateConfig (cmd: UpdateConfig) =
    ...
    | true, Some file ->
        ...
        executeNonQuery session <| queries.["fl_updateConfig"].Bind({| data = configText; key = "config" |});
...
let runUpdateFromDatabase () = runUpdateConfig { database = false; keyspace = None; jsonConfig = None }
